...
export function HomepageContentManager() {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      sponsorTitle: "Sponsors / Collaborators",
      sponsorSubtitle: "", sponsorLocation: "", sponsorDescription: "", sponsorWhatsapp: "", sponsorInstagram: "", sponsorImageUrl: "",
      sponsors: [],
  ...
  useEffect(() => {
    if (settingsDoc) {
      ...
      // Migration: If no sponsors array but old singular data exists, create one entry.
      let existingSponsors = data.sponsors || [];
      if (existingSponsors.length === 0 && (data.sponsorSubtitle || data.sponsorLocation || data.sponsorImageUrl)) {
        existingSponsors = [{
          title: data.sponsorTitle || "Main Sponsor", // Use common title or specific
          subtitle: data.sponsorSubtitle || "",
          location: data.sponsorLocation || "",
          description: data.sponsorDescription || "",
          whatsappUrl: data.sponsorWhatsapp || "",
          instagramUrl: data.sponsorInstagram || "",
          imageUrl: data.sponsorImageUrl || "",
        }];
      }
      ...
      form.reset({
        ...
        // Keep singular for now to avoid validation errors if we switch back, but UI will focus on array
        sponsorSubtitle: data.sponsorSubtitle || "",
        sponsorLocation: data.sponsorLocation || "",
        sponsorDescription: data.sponsorDescription || "",
        sponsorWhatsapp: data.sponsorWhatsapp || "",
      ...
      // setSponsorPhotoPreview(data.sponsorImageUrl || null); // No longer relevant for singular if using list
    }
  }, [settingsDoc, form]);
  ...
  const handleSponsorListPhotoChange = async (event: React.ChangeEvent<HTMLInputElement>, index: number) => {
    const file = event.target.files?.[0];
    if (file) {
      setIsUploading(true);
      try {
        ...
        form.setValue(`sponsors.${index}.imageUrl`, url, { shouldValidate: true });
        toast({ title: 'Success', description: 'Sponsor image uploaded.' });
      } catch (e) {
        toast({ variant: 'destructive', title: 'Upload Failed', description: (e as Error).message });
      } finally {
        setIsUploading(false);
        ...
        {isLoading ? (
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="space-y-4 pt-4 border-t">
                <div className="flex items-center justify-between border-b pb-2">
                  <h3 className="font-semibold text-lg">Sponsors / Collaborators</h3>
                  <Button type="button" variant="outline" size="sm" onClick={() => appendSponsor({ title: "New Sponsor", subtitle: "", location: "", description: "", whatsappUrl: "", instagramUrl: "", imageUrl: "" })}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FormField name="sponsorTitle" control={form.control} render={({ field }) => (
                    <FormItem><FormLabel>Section Title (e.g. Our Sponsors)</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                  )} />
                </div>
                ...
                <div className="space-y-2">
                  <Accordion type="single" collapsible className="w-full space-y-2">
                    {sponsorFields.map((field, index) => (
                      <AccordionItem value={field.id} key={field.id} className="border rounded-lg bg-card px-4">
                        <AccordionTrigger className="hover:no-underline hover:bg-accent/50 rounded-lg px-2 -mx-2">
                          <div className="flex items-center gap-4 text-left">
                            {/* Using form.watch for live updates */}
                            {form.watch(`sponsors.${index}.imageUrl`) ? (
                              <div className="w-10 h-10 relative rounded-full overflow-hidden border bg-background">
                                <Image src={form.watch(`sponsors.${index}.imageUrl`)} alt="Logo" fill className="object-contain" />
                              </div>
                            ) : <div className="w-10 h-10 rounded-full bg-muted flex items-center justify-center text-xs">IMG</div>}
                            <div className="flex flex-col">
                              <span className="font-semibold text-sm">{form.watch(`sponsors.${index}.title`) || "New Sponsor"}</span>
                              <span className="text-xs text-muted-foreground truncate max-w-[200px]">{form.watch(`sponsors.${index}.subtitle`)}</span>
                            </div>
                          </div>
                        </AccordionTrigger>
                        <AccordionContent className="pt-4 pb-4 space-y-4 border-t mt-2">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <FormField name={`sponsors.${index}.title`} control={form.control} render={({ field }) => (
                              <FormItem><FormLabel>Name / Title</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                            )} />
                            <FormField name={`sponsors.${index}.subtitle`} control={form.control} render={({ field }) => (
                              <FormItem><FormLabel>Subtitle (e.g. Gold Partner)</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                            )} />
                          </div>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <FormField name={`sponsors.${index}.location`} control={form.control} render={({ field }) => (
                              <FormItem><FormLabel>Location</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                            )} />
                            <FormField name={`sponsors.${index}.whatsappUrl`} control={form.control} render={({ field }) => (
                              <FormItem><FormLabel>WhatsApp Link</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                            )} />
                          </div>
                          <FormField name={`sponsors.${index}.description`} control={form.control} render={({ field }) => (
                            <FormItem><FormLabel>Description</FormLabel><FormControl><Textarea {...field} rows={2} /></FormControl><FormMessage /></FormItem>
                          )} />
                          <FormField name={`sponsors.${index}.instagramUrl`} control={form.control} render={({ field }) => (
                          ...
                          <FormItem>
                            ...
                            <FormControl>
                              <div className="flex items-center gap-4">
                                {form.getValues(`sponsors.${index}.imageUrl`) && (
                                  <div className="relative w-24 h-16 border rounded bg-muted overflow-hidden">
                                    <Image src={form.getValues(`sponsors.${index}.imageUrl`)} alt="Preview" fill className="object-contain" />
                                  </div>
                                )}
                                <div className="flex-1">
                                  <Input type="file" accept="image/*" onChange={(e) => handleSponsorListPhotoChange(e, index)} disabled={isUploading} />
                                  <p className="text-xs text-muted-foreground mt-1">Upload a logo or image.</p>
                                </div>
                              </div>
                            </FormControl>
                          </FormItem>
                          ...
                          <div className="flex justify-end pt-2">
                            <Button type="button" variant="destructive" size="sm" onClick={() => {
                              removeSponsor(index);
                              // We also need to clear the singular fields in form state so they don't resurrect the sponsor on next load/save
                              if (sponsorFields.length <= 1) { // length before removal
                                form.setValue('sponsorTitle', '');
                                form.setValue('sponsorSubtitle', '');
                                form.setValue('sponsorLocation', '');
                                form.setValue('sponsorDescription', '');
                                form.setValue('sponsorWhatsapp', '');
                                form.setValue('sponsorInstagram', '');
                                form.setValue('sponsorImageUrl', '');
                              }
                            }}>
                              <Trash2 className="h-4 w-4 mr-2" /> Delete Sponsor
                            </Button>
                          </div>
                        </AccordionContent>
                      </AccordionItem>
                  ...
                  {sponsorFields.length === 0 && <p className="text-sm text-muted-foreground text-center py-6 border rounded-lg border-dashed">No sponsors added via list.</p>}
                </div>
              </div>
