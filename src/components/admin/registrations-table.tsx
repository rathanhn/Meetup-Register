...
export function RegistrationsTable() {
    ...
    const handleUpdateStatus = async (id: string, status: 'approved' | 'rejected') => {
        ...
        try {
            ...
            const result = await updateRegistrationStatus({ registrationId: id, status, adminId: user.uid, token });
            if (result.success) {
    ...
    const handleCancellation = async (id: string, approve: boolean) => {
        ...
        try {
            const token = await user.getIdToken();
            if (approve) {
                const result = await updateRegistrationStatus({ registrationId: id, status: 'cancelled', adminId: user.uid, token });
                if (result.success) {
                ...
                const result = await updateRegistrationStatus({ registrationId: id, status: 'approved', adminId: user.uid, token });
                if (result.success) {
